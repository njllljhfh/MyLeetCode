...
def quickSort(arr, left=None, right=None, reverse=False):
    """
    业务代码不用传递 left 和 right，这两个参数是内部递归用的
    :param arr:
    :param left:
    :param right:
    :param reverse: 是否倒序
    ...
    if left < right:  # 要排序部分的长度 大于1 才需要排序
        partitionIndex = partition(arr, left, right, reverse=reverse)
        quickSort(arr, left, partitionIndex - 1, reverse=reverse)
        quickSort(arr, partitionIndex + 1, right, reverse=reverse)
    return arr
